...
- **Hero mobile (`Hero.tsx`)**: foto LCP = `maria-padilha-hero.webp` (não PNG 1.7MB). Canvas transparente largo → mobile `width: 148vw`, `right-[-22vw]`, `object-[74%_bottom]` (tall `84%`). Content `max-lg:pr-[28vw]` (sm `24vw` / md `20vw`). Lead mobile: `w-[calc(100vw-3rem)]` + `max-w-none` (quase full width). **Nunca** `initial={{ opacity: 0 }}` na foto hero — atrasa LCP no Lighthouse mobile. `sizes` mobile ≈ `100vw` (não `148vw`) pra não baixar imagem gigante.
