...
- **Hero mobile (`Hero.tsx`)**: foto LCP = `maria-padilha-hero.png` (prod). Props opcionais `imageSrc` / `imageWidth` / `imageHeight`. Canvas transparente largo → mobile `width: 148vw`, `right-[-22vw]`, `bottom-0` + `origin-bottom scale-[1.14]` (zoom grounded), `object-[74%_bottom]` (tall `84%`). Content `max-lg:pr-[28vw]` (sm `24vw` / md `20vw`). Lead mobile: `w-[calc(100vw-3rem)]` + `max-w-none` (quase full width). **Nunca** `initial={{ opacity: 0 }}` na foto hero — atrasa LCP no Lighthouse mobile. `sizes` mobile ≈ `100vw` (não `148vw`) pra não baixar imagem gigante.
- **LP variante look roxo**: rota `/lp-roxo` — mesma home, hero = `maria-padilha-look-roxo.webp` (878×1792, ~91KB). PNG fonte em `web/public/maria-padilha-look-roxo.png` + original na raiz do repo. `robots: noindex` + canonical `/` (não compete SEO com home).
