...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [...new Set([...post.tags, post.category, "Amélia Saúde", "plano de saúde", "Rio de Janeiro"])].join(", "),
  };
}
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const postUrl = `${SITE_URL}/blog/${post.slug}`;
  const pubDate = parseBlogDate(post.date);
    ...
    <>
      <StructuredData
        ...
        data={{
          ...
          author: { name: "Amélia Saúde" },
          publisher: { name: "Amélia Saúde", logo: `${SITE_URL}/logo-amelia-site.png` },
          url: postUrl,
          ...
          <article className="mx-auto max-w-[680px] px-4" style={{ padding: "clamp(3rem, 6vh, 5rem) clamp(1.5rem, 5vw, 5rem)" }}>
            ...
            <nav aria-label="Breadcrumb" className="mb-8 font-sans text-sm text-[#8a8a8a]">
              <ol className="flex items-center gap-2">
                <li><Link href="/" className="hover:text-[#7b6bb2] transition-colors">Home</Link></li>
                <li aria-hidden="true">/</li>
                <li><Link href="/blog" className="hover:text-[#7b6bb2] transition-colors">Blog</Link></li>
                <li aria-hidden="true">/</li>
                <li className="text-[#5e4985] truncate max-w-[200px]" aria-current="page">{post.title}</li>
              </ol>
            </nav>
            ...
            <div className="mt-6 flex items-center gap-4 font-sans text-sm text-[#8a8a8a]">
              <time dateTime={pubDate.toISOString().split("T")[0]}>{post.date}</time>
