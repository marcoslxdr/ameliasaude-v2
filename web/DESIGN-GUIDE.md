...
**Fundos de seção (home):** usar [`HeroBackground`](src/components/ui/HeroBackground.tsx) com `variant` sutil (`hero`, `origin`, `bloom`, `haze`, `veil`) — mesmo gradiente/trama/grain da marca, glow e ângulo variam por seção. Conteúdo com `relative z-10` acima do fundo.
